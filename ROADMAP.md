...
| **M23** · Ecosystem (Spring / LangChain4j / Koog) + RAG demo | 🚧 In progress (Spring Boot starter done). |
...
bean; verified with an `ApplicationContextRunner` test). **Remaining:** `kdrant-spring-ai` (`VectorStore`),
`kdrant-langchain4j` (`EmbeddingStore`), `kdrant-koog`, and the runnable RAG demo app.
